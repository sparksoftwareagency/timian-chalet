      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          <div className="space-y-4">
            <div>
              <Image src={settings.logoLightUrl} alt={settings.siteTitle} width={180} height={36} className="h-9 w-auto" />
            </div>
            ...
            <ul className="space-y-2">
              {settings.quickLinks.map((item) => (
                <li key={`${item.href}-${item.label}`}>
                  <Link href={localizeHref(locale, item.href)} className="text-sm hover:opacity-80 transition-opacity" style={{ color: colors.secondaryBg }}>
                    {item.label}
                  </Link>
                </li>
              ))}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center space-x-3">
                ...
                <a href={`tel:${settings.phone}`} className="text-sm hover:opacity-80 transition-opacity" style={{ color: colors.secondaryBg }}>
                  {settings.phone}
                </a>
              </div>
              <div className="flex items-center space-x-3">
                ...
                <a href={`mailto:${settings.email}`} className="text-sm hover:opacity-80 transition-opacity" style={{ color: colors.secondaryBg }}>
                  {settings.email}
                </a>
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center space-y-2 sm:space-y-0">
            ...
            <div className="flex space-x-6">
              {settings.legalLinks.map((link) => (
                <Link key={`${link.href}-${link.label}`} href={localizeHref(locale, link.href)} className="text-sm hover:opacity-80 transition-opacity" style={{ color: colors.secondaryBg }}>
                  {link.label}
                </Link>
              ))}
            </div>
